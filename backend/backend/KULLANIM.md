...
> **Güvenlik:** JWT etkin. `/auth/kayit`, `/auth/giris` ve GET restoran/menü okuma herkese açık; `/chat`, `/oneri`, `/karsilastir`, `/kullanici/*` ve AI analiz uçları **Bearer token** gerektirir. Menü yazma uçları **RESTORAN** rolü ister.

---

## 1. Bu sistem ne işe yarıyor?

- **Kayıt / giriş:** E-posta ve şifre ile hesap; yanıtta JWT `token` döner.
- **Chatbot / öneri / karşılaştırma:** `GROQ_API_KEY` ortam değişkeni gerekir ([Groq Console](https://console.groq.com/keys), ücretsiz kota).
- **Restoran ve menü:** Okuma herkese açık; menü yazma RESTORAN rolü ile.

Şifre **JSON gövdesinde** gider; korumalı uçlarda **Bearer Token** kullanılır.
...
## 8. Yapay zekâ senaryoları (dokümandaki hikâyeler)

### Mert — Bütçe odaklı chatbot

```bash
curl -s -X POST "http://localhost:8080/pick-a-bite/chat" \
  -H "Authorization: Bearer YOUR_JWT" \
  -H "Content-Type: application/json" \
  -d '{"mesaj":"250 TL bütçem var, tavuklu bir yemek öner","enlem":41.015,"boylam":28.98}'
```

### Zeynep — Çoklu restoran karşılaştırma

```bash
curl -s -X POST "http://localhost:8080/pick-a-bite/karsilastir" \
  -H "Authorization: Bearer YOUR_JWT" \
  -H "Content-Type: application/json" \
  -d '{"mesaj":"300 TL altında sağlıklı yemek öner","enlem":41.015,"boylam":28.98}'
```

### Can — QR ile menü

`GET /pick-a-bite/restoranlar/qr/{qrKod}` — geçersiz kodda: *"Bu QR kod sisteme kayıtlı bir restorana ait değil."*

### Menü JSON senkronu

```bash
export GROQ_API_KEY=gsk_...   # AI uçları için
curl -s -X POST "http://localhost:8080/pick-a-bite/admin/menu-senkron" \
  -H "Authorization: Bearer YOUR_JWT"
```
...
**“503 AI servisi”**  
- `export GROQ_API_KEY=gsk_...` ayarlayıp uygulamayı yeniden başlatın.

## 10. İngilizce teknik özet
